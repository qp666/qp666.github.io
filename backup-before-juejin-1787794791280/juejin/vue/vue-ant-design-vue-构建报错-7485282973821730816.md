...
模块结构设计：Ant Design Vue 将某些组件设计为父组件的子属性，而不是独立的模块

打包优化：构建工具在打包时会进行模块解析优化，直接导入不存在的子模块会报错

Tree Shaking：打包工具的摇树优化可能会移除未正确引用的模块
...
统一导入方式：在项目中统一使用父组件属性的方式

类型安全：使用 TypeScript 时确保类型定义正确

代码可读性：优先选择直接使用 Component.SubComponent 的方式
