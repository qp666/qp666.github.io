...
![image.png](https://p9-xtjj-sign.byteimg.com/tos-cn-i-73owjymdk6/54ece61f9d1046fb9ae991e1c3519df0~tplv-73owjymdk6-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAg5YmN56uv5Y2_5bm0:q75.awebp?rk3s=f64ab15b&x-expires=1788167706&x-signature=jlW7e0zR7MpmPc3ouA%2BONd49xjc%3D)
...
1.  **状态跟踪**：使用 `shiftKeyDown` 状态记录 Shift 键是否处于按下状态
2.  **按键监听**：分别监听 `keydown` 和 `keyup` 事件
3.  **状态更新**：
    -   Shift 键按下时，设置 `shiftKeyDown = true`
    -   Shift 键释放时，设置 `shiftKeyDown = false`
4.  **组合判断**：Enter 键按下时，根据 `shiftKeyDown` 状态判断是否为组合键
...
1.  **焦点丢失**：当输入框失去焦点时，需要重置 `shiftKeyDown` 状态
2.  **页面切换**：页面切换时需要清理状态，避免状态残留
3.  **事件绑定**：确保 `keyup` 和 `keydown` 事件都正确绑定
