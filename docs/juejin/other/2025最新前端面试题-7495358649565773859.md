...
tag:
  ...
  - 问题排查
  - 前端
source: https://juejin.cn/post/7495358649565773859
---
...
### template跟render有什么区别

```arduino
Vue 中，template 和 render 是两种视图渲染入口，
核心区别在于「模板编译时机」和「语法形态」：
template 是声明式模板，需先编译为 render 函数再执行；
render 是函数式 API，直接返回虚拟 DOM（VNode），无需编译。

从源码角度分析，Vue 编译 template 时，
会调用 compileToFunctions 函数将模板字符串转换为 render 函数。
render 函数执行时，会根据组件状态动态生成 VNode 树，最终渲染到页面。

总结：template 是声明式模板，render 是函数式 API，
两者在 Vue 中扮演不同角色，分别处理不同的视图渲染场景。
```
...
```vbnet
前端视角的 “面向对象（OOP）”：用 “对象” 打包逻辑
面向对象的核心思想是 “将数据（状态）和操作数据的方法（行为）封装在一个‘对象’中”，
通过类（Class）定义对象的模板，再实例化使用。
前端中 OOP 的应用主要集中在 框架设计、复杂组件封装、工具类实现 等场景。

前端视角的 “Hooks”：用 “函数” 拆分独立逻辑
Hooks 是 “将组件中的独立逻辑
（如状态管理、生命周期、副作用）抽离为可复用的函数”，
核心是 “函数式复用”，而非 “对象封装”。
它是 React 16.8+、Vue3 等框架推出的方案，专门解决 OOP/Mixin 带来的逻辑复用问题。

一句话总结
OOP：“把相关的东西（状态 + 方法）打包成一个对象，方便调用和管理”；
Hooks：“把独立的逻辑（如请求、监听）抽成一个函数，方便复用和维护”
```

### 原型链怎么获取到构造函数的名字

```kotlin
推荐：instance.constructor.name（ES6+ 简洁方案）
ES6 为函数新增了 name 属性，直接返回构造函数的名称字符串，通过原型链的 constructor 可快速获取
